...
function getPieDimensions(mdObject: VisualizationObject.IVisualizationObjectContent): AFM.IDimension[] {
    ...
    if (view && view.items.length) {
        return [
            ...
            {
                itemIdentifiers: view.items.map((attribute: VisualizationObject.IVisualizationAttribute) =>
        ...
        return [
            ...
            },
            {
                itemIdentifiers: (view && view.items || [])
                    .map((v: VisualizationObject.IVisualizationAttribute) =>
                        v.visualizationAttribute.localIdentifier)
            }
        ];
    }
    ...
    return [
        {
            itemIdentifiers: (stack && stack.items || [])
                .map((s: VisualizationObject.IVisualizationAttribute) => s.visualizationAttribute.localIdentifier)
        },
        {
            itemIdentifiers: ((view && view.items || [])
                .map((v: VisualizationObject.IVisualizationAttribute) => v.visualizationAttribute.localIdentifier))
                .concat([MEASUREGROUP])
        }
    ];
}

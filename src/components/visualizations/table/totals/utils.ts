...
import { IAlignPoint, ITotalsDataSource, ITotalTypeWithTitle, TableHeader } from '../interfaces/data';
...
function getTotalsList(intl: InjectedIntl): ITotalTypeWithTitle[] { // TODO any
...
export function getTotalsDataSource(usedTotals: ITotalWithData[], intl: any): ITotalsDataSource { // TODO any
...
export function toggleCellClass(
    ...
    Array.from(cells).forEach((cell: any) => { // TODO any (should beTableCell?)
        if (isHighlighted) {
...
export function updateTotalsRemovePosition(
    ...
    const translateY: number = tableBoundingRect.height - getFooterHeight(totals, isTotalsEditAllowed, totalsVisible);
...
export function shouldShowAddTotalButton(
    ...
    return !isFirstColumn && header.type === 'measure' && addingMoreTotalsEnabled;
}

export function getFirstMeasureIndex(headers: TableHeader[]): number {
    const measureOffset = headers.findIndex(header => header.type === 'measure');
...
export function shouldShowTotals(headers: TableHeader[]): boolean {
    ...
    const onlyMeasures: boolean = headers.every((header: TableHeader) => header.type === 'measure');
    const onlyAttributes: boolean = headers.every((header: TableHeader) => header.type === 'attribute');

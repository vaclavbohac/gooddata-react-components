...
import {
    Align, IAlignPoint, IScrollEvent, ISortObj, SortDir, TableCell, TableHeader,
    TableRow
...
export class TableVisualizationClass
    ...
    private subscribers: any; // TODO any
    ...
    private unsetListeners(): void {
        if (this.subscribers && this.subscribers.length > 0) {
            this.subscribers.forEach((subscriber: any) => { // TODO any
    ...
    private scrollHeader(isScrollingStopped: boolean = false): void {
        ...
        // TODO any below
        const positions: any = getHeaderPositions(
            stickyHeaderOffset,
            hasHiddenRows,
            totalsWithData,
            totalsEditAllowed,
    ...
    private scrollFooter(isScrollingStopped: boolean = false): void {
        ...
        // TODO any below
        const positions: any = getFooterPositions(
            hasHiddenRows,
            totalsWithData,
            window.innerHeight,
            totalsEditAllowed,
    ...
    private addTotalsRow(columnIndex: number, totalType: AFM.TotalType): void { // TODO create totalType type
    ...
    private renderTooltipHeader(header: TableHeader, columnIndex: number, columnWidth: number): any { // TODO any
    ...
    private renderDefaultHeader(header: TableHeader, columnIndex: number): any { // TODO any
    ...
    private renderCell(headers: TableHeader[], columnIndex: number): any { // TODO any
        ...
        return (cellProps: any) => { // TODO any
            ...
            const drillConfig: any = { afm, onFiredDrillEvent }; // TODO any
            const hoverable: boolean = header.type === 'measure' && this.isTotalsEditAllowed();
            const { style, label } = getStyledLabel(header, cellContent);

            // TODO continue here
            const cellPropsDrill: any = drillable // TODO any
                ? assign({}, cellProps, {
                    onClick(e: any) { // TODO any
                        cellClick(
                            drillConfig,
                            {
            ...
            const cellPropsHover: any = hoverable // TODO any
                ? assign({}, cellPropsDrill, {
    ...
    private renderFooter(header: TableHeader, columnIndex: number, headersCount: number): any { // TODO any
        const { headers, totalsWithData } = this.props;
        ...
        const onCellMouseLeave = (rowIndex: number, colIndex: number) => { // TODO unused
            this.resetTotalsRowHighlight(rowIndex);
            this.toggleFooterColumnHighlight(colIndex, false);
        };

        const onAddDropdownOpenStateChanged = (colIndex: number, isOpened: boolean) => { // TODO lambda
        ...
        const onAddWrapperHover = (colIndex: number, isHighlighted: boolean) => { // TODO lambda
            this.toggleFooterColumnHighlight(colIndex, isHighlighted);
        };

        const onAddButtonHover = (colIndex: number, isHovered: boolean) => { // TODO lambda
    ...
    private renderColumns(headers: TableHeader[], columnWidth: number): any { // TODO any
        const renderHeader: Function = this.props.sortInTooltip // TODO type function
    ...
    private renderStickyTableBackgroundFiller(): any { // TODO any
        return (
            <div
    ...
    private renderTotalsRemove(): any { // TODO any
        if (!this.isTotalsEditAllowed()) {
            return false;
        }

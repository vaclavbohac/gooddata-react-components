...
import {
    TableVisualization, DEFAULT_FOOTER_ROW_HEIGHT, ITableVisualizationProps,
    ITableVisualizationState
} from '../TableVisualization';
import { TableHeader, TableRow } from '../interfaces/data';
...
import { withIntl, wrapWithIntl } from '../../common/utils/intlUtils';
...
function createPortalWrapper(tableWrapper: ReactWrapper<any>): ReactWrapper<any> { // TODO any
    const portalInstance: React.Component<any> = tableWrapper.find(Portal).at(0).instance(); // TODO any
...
describe('Table', () => {
    ...
    it('should sort by clicking on button in tooltip', () => {
        ...
        const header =  wrapper.find('.gd-table-header-ordering').at(0); // TODO any
        header.simulate('click');
        return testUtils.delay().then(() => {
            expect(onSortChange).toHaveBeenCalledTimes(0);
            const portalWrapper: ReactWrapper<any> = createPortalWrapper(wrapper); // TODO any
            const btn: any = portalWrapper.find('.button-sort-asc'); // TODO any
            btn.simulate('click');
    ...
    it('should align measure columns to the right', () => {
        const wrapper: ReactWrapper<ITableVisualizationProps & InjectedIntlProps, ITableVisualizationState> =
            renderTable();
        const columns: any = wrapper.find(Table).prop('children'); // TODO any
    ...
    describe('renderers', () => {
        function renderCell(
            ...
            const columns: any = wrapper.find(Table).prop('children'); // TODO any
            const cell: any = columns[columnKey].props.cell({ rowIndex: 0, columnKey }); // TODO any
        ...
        it('should format measures', () => {
            const wrapper: ReactWrapper<ITableVisualizationProps & InjectedIntlProps, ITableVisualizationState> =
                renderTable();
            const span: any = renderCell(wrapper, 2); // TODO any
            const spanContent: any = span.props.children; // TODO any
        ...
        it('should render attributes as strings', () => {
            const wrapper: ReactWrapper<ITableVisualizationProps & InjectedIntlProps, ITableVisualizationState> =
                renderTable();
            const span: any = renderCell(wrapper, 0);
            const spanContent: any = span.props.children;
        ...
        it('should bind onclick when cell drillable', () => {
            ...
            const columns: any = wrapper.find(Table).prop('children');
            const cell: any = columns[1].props.cell({ rowIndex: 0, columnKey: 1 });
    ...
    describe('sort', () => {
        describe('default header renderer', () => {
            it('should render up arrow', () => {
                ...
                const columns: any = wrapper.find(Table).prop('children');
                const header: any = columns[0].props.header({ columnKey: 0 });
                const sort: any = header.props.children[1];
            ...
            it('should render arrow on second column', () => {
                ...
                const columns: any = wrapper.find(Table).prop('children');
                const header: any = columns[1].props.header({ columnKey: 0 });
                const sort: any = header.props.children[1];
        ...
        describe('tooltip header renderer', () => {
            it('should render title into header', () => {
                ...
                const bubble: any = document.querySelector('.gd-table-header-bubble');
    ...
    describe('table footer and totals', () => {
        ...
        describe('totals edit not allowed', () => {
            ...
            it('should update footer height when component is updated with different totals', () => {
                ...
                const { footer }: { footer: any } = getInstanceFromWrapper(wrapper, TableVisualization);

                const heightBefore: number = TOTALS.length * DEFAULT_FOOTER_ROW_HEIGHT;

                expect(footer.style.height).toEqual(`${heightBefore}px`);

                const totalsAfter: any = [...TOTALS, { // TODO any -> totalsWithData
                    type: 'min',
                    outputMeasureIndexes: [],
                    values: [1, 2, 3]
                }];
        ...
        describe('totals edit allowed', () => {
            ...
            it('should bind mouse events on table body cells', () => {
                ...
                const component: any = wrapper.find(TableVisualization).childAt(0).instance();
                const cell: any = wrapper.find('.fixedDataTableCellLayout_wrap1.col-2').at(0);
